...
const FeatureBlock: React.FC<FeatureBlockProps> = ({ feature }) => {
  ...
  return (
    <div className={`flex flex-col md:flex-row items-center gap-12 py-12 md:py-24 ${!isLeft ? 'md:flex-row-reverse' : ''}`}>
      {/* Image Side */}
      <div className="w-full md:w-1/2 relative">
        <div className={`absolute top-0 ${isLeft ? '-left-10' : '-right-10'} w-full h-full text-brand-lightPurple z-0 opacity-60 scale-125`}>
        ...
        <div className="relative z-10 rounded-[2rem] overflow-hidden shadow-2xl border-4 border-white transform hover:scale-[1.02] transition-transform duration-500">
           ...
           <div className={`absolute bottom-6 ${isLeft ? '-right-6' : '-left-6'} bg-white p-4 rounded-xl shadow-lg max-w-[200px] hidden lg:block animate-bounce duration-[3000ms]`}>
        ...
        <ul className="space-y-3">
            {feature.tags.map((tag, idx) => (
                <li key={idx} className="flex items-center gap-3 text-gray-700 font-medium">
                    <CheckCircle2 className="text-brand-orange flex-shrink-0" size={20} />
                    {tag}
                </li>
            ))}
        </ul>
